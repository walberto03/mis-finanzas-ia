...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'finance-app-advanced';

// Helper para formato de moneda compacto
...
const NetworkExplorer = ({ messages, onSelectTag }) => {
  ...
  const { nodes, links, maxValue } = useMemo(() => {
    ...
    if (!centerTag) {
      ...
      // Posicionar en círculo grande
      sortedNodes.forEach((n, i) => {
        const angle = (i / sortedNodes.length) * 2 * Math.PI - (Math.PI / 2); // Empezar arriba
  ...
  return (
    <div className="relative w-full h-[450px] bg-slate-900 rounded-3xl overflow-hidden shadow-2xl flex items-center justify-center border border-slate-800">
      ...
      <svg viewBox="0 0 100 100" className="w-full h-full p-2 select-none">
        <defs>
          {/* Definir gradientes para esferas 3D */}
        ...
        {/* Nodos (Burbujas) */}
        {nodes.map((node) => {
          // Calcular radio basado en valor (min 6, max 16)
          ...
          return (
            ...
            >
              ...
              <circle 
                ...
                fill={fillId} 
                stroke="white" strokeWidth="0.3" strokeOpacity="0.3"
              />
              
              {/* Texto: Nombre */}
              <text 
                x={node.x} y={node.y - (radius * 0.2)} 
                textAnchor="middle" 
                fill="white" 
                fontSize={Math.max(2.5, radius * 0.35)}
                fontWeight="bold"
              ...
              {/* Texto: Valor */}
              <text 
                x={node.x} y={node.y + (radius * 0.5)} 
                textAnchor="middle" 
                fill="rgba(255,255,255,0.9)" 
                fontSize={Math.max(2, radius * 0.3)}
                fontWeight="500"
...
export default function FinanceApp() {
  ...
  // Mensajes para la lista de detalle
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <main className="flex-1 overflow-y-auto bg-slate-50/50">
              ...
              <div key={msg.id} className={`flex flex-col animate-in slide-in-from-bottom-2 duration-300 ${msg.sender === 'Yo' ? 'items-end' : 'items-start'}`}>
                <div className={`flex items-end gap-2 max-w-[90%] ${msg.sender === 'Yo' ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className={`p-3 rounded-2xl shadow-sm border ${
                    msg.sender === 'Yo' 
                      ? 'bg-white border-indigo-50 rounded-tr-none' 
                      : 'bg-white border-pink-50 rounded-tl-none'
                  }`}>
                    ...
                    <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 min-w-[200px]">
                      <div className="flex justify-between items-start mb-2">
                        <span className={`text-xs font-bold px-2 py-0.5 rounded ${
                          msg.type === 'income' ? 'bg-green-100 text-green-700' : 
                          msg.type === 'debt_payment' ? 'bg-orange-100 text-orange-700' : 
                      ...
                      <div className="flex flex-wrap gap-1">
                        {msg.tags?.map((tag, idx) => (
                          <span key={idx} className="text-[10px] bg-white border border-slate-200 text-slate-500 px-1.5 py-0.5 rounded-md flex items-center gap-1">
                            <Tag size={10} /> {tag}
                          </span>
                        ))}
                      </div>
                  ...
                  <button onClick={() => handleDelete(msg.id)} className="text-slate-300 hover:text-red-400 p-1">
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
        ...
        {/* VISTA 2: EXPLORAR (MEJORADA) */}
        {activeTab === 'explore' && (
          <div className="p-4 space-y-6 pb-24">
             <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 mb-4">
               <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                 <Network className="text-indigo-600" /> Mapa de Gastos
               </h2>
               <p className="text-xs text-slate-500">
                 Visualización interactiva de tu ecosistema financiero.
               </p>
             </div>

             {/* COMPONENTE DE RED 3D */}
             <NetworkExplorer 
                ...
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-4">
                  <div className="p-3 bg-indigo-50 border-b border-indigo-100 flex justify-between items-center">
                    ...
                    <span className="text-xs text-indigo-500 font-mono">
                      Total: ${messagesForTagModal.reduce((sum, m) => sum + (m.amount||0), 0).toLocaleString()}
                    </span>
                  </div>
        ...
        {activeTab === 'stats' && (
          <div className="p-4 space-y-6 pb-24">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
              ...
              <div className="flex gap-2 items-center">
                <input 
                  ...
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-2 py-2 text-xs"
                />
                <input 
                  type="date" 
                ...
                {(dateFilter.start || dateFilter.end) && (
                  <button onClick={() => setDateFilter({ start: '', end: '' })} className="p-2 bg-slate-100 rounded-lg"><X size={14}/></button>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-2 mb-1 text-green-600">
                  <TrendingUp size={16} /> <span className="text-xs font-bold uppercase">Ingresos</span>
                </div>
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                <div className="flex items-center gap-2 mb-1 text-red-500">
                  <TrendingDown size={16} /> <span className="text-xs font-bold uppercase">Egresos</span>
                </div>
            ...
            <div className={`p-4 rounded-2xl shadow-sm border flex justify-between items-center ${balance >= 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-red-50 border-red-100'}`}>
              <div>
                <p className="text-xs font-bold uppercase mb-1 opacity-70">Liquidez</p>
                <h2 className={`text-3xl font-bold ${balance >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>
                ...
                {sortedTags.length > 0 ? sortedTags.slice(0, 10).map((tag, idx) => (
                  <div key={tag.name} className="flex justify-between items-center text-sm">
                      ...
                      <div className="flex-1 mx-2 h-2 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-indigo-500 rounded-full" style={{ width: totalExpense > 0 ? `${(tag.value / totalExpense) * 100}%` : '0%' }} />
                      </div>
      ...
      <nav className="bg-white border-t border-slate-100 flex justify-around p-1 z-30 pb-safe">
        <button onClick={() => setActiveTab('chat')} className={`p-3 rounded-xl transition-all ${activeTab === 'chat' ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400'}`}>
          <MessageSquare size={24} />
        </button>
        <button onClick={() => setActiveTab('explore')} className={`p-3 rounded-xl transition-all ${activeTab === 'explore' ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400'}`}>
          <Network size={24} />
        </button>
        <button onClick={() => setActiveTab('stats')} className={`p-3 rounded-xl transition-all ${activeTab === 'stats' ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400'}`}>
          <PieIcon size={24} />
        </button>
      </nav>
